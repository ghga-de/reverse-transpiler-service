...
```bash
docker pull ghga/reverse-transpiler-service:2.2.1
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/reverse-transpiler-service:2.2.1 .
```
...
docker run -p 8080:8080 ghga/reverse-transpiler-service:2.2.1 --help
```
